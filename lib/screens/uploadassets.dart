                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      ...
                      (image == null)
                          ? Container()
                          : imageBox(context, File(image!.path)),
                    ],
                  ),
                ],
              ),
                    ...
                    try {
                      if (_docIdKey.currentState!.validate()) {
                        ...
                        await FirebaseStorage.instance
                            .ref("$docId/images/$_filename")
                            .putFile(_file)
                            .whenComplete(() async {
                          String _downloadUrl = await FirebaseStorage.instance
                              .ref("$docId/images/$_filename")
                              .getDownloadURL();
                          print("downloadURL : $_downloadUrl");
                          //Upload its link to Firestore
                          await FirebaseFirestore.instance
                              .collection("$_dropdownvalue-record")
                              .doc(docId)
                              .set({
                            'images': {
                              'img-$_filename': '$_downloadUrl',
                            },
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content:
                                Text("Enter CatererID or BanquetID number"),
                          ),
                        );
                      }
                      ...
                      if (e.code == 'unauthorized') {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text("ImageSize<=1MB / VideoSize<=2MB"),
                          ),
                        );
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                                "Wrong location set.Check if the value is Banquet or Caterer or  Please check if file size is less than 1 MB and try again."),
                          ),
                        );
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
